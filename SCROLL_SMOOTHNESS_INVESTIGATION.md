...
- **Chromium switches**: `--enable-smooth-scrolling`, `--enable-gpu-rasterization`, `--enable-zero-copy`, `--ignore-gpu-blocklist`. None of these moved the needle on their own; we left them because they're cheap and orthogonal.
...
- **Diagnostic instrumentation** in `src/paint.ts` behind the `-p` / `--debug-paint` flag. Logs `dt`, `tb` (toBitmap), `sw` (stdout to Kitty), and dimensions per paint. **Keep this.** It saved us from many speculative fixes and will be needed again next time.
- **`ShmGraphicBuffer` keeps shm-name rotation and `ftruncate` EINVAL tolerance** — both load-bearing if anyone re-enables `useSharedTexture` or any other path that produces faster paint rates. See macOS POSIX shm section below.
- **Dead-but-kept code**: `write_iosurface` Rust method, IOSurface FFI bindings, `build.rs` framework links, and the `useSharedTexture` paint-handler branch in `paint.ts`. We left these in for the next attempt.
...
On Electron 41, plain `offscreen: true`, post-upgrade: similar median but somewhat smoother in feel. The exact spike rate wasn't re-measured but is likely similar.
...
The remaining unbridged perf gap is the 5–8 ms `tb` of `toBitmap()` plus the 12–15% paint-event spike rate. Worth attempting:

1. **Metal-based texture readback** instead of `IOSurfaceLock`. Use `objc2-metal` to wrap the IOSurface as an `MTLTexture`, blit it to a CPU-readable `MTLBuffer` with `storageModeShared`, `commandBuffer.waitUntilCompleted()`, then read. This avoids the IOSurfaceLock entirely. The `bgra-to-rgba` work could move to a Metal compute shader. ~150-250 lines of `unsafe` Rust. Likely produces ~5 ms `tb` (similar to `toBitmap`) but with no GPU sync stalls — the spikes might disappear. Caveat: still has the premultiplied-alpha + colorSpace problem from problem D, so you'd also need to do the unpremultiply step (compute shader, cheap).
2. **Wait for Electron 42+** for OSR's `deviceScaleFactor` option (per Electron 42 release notes — we're on 41) and any further OSR pipeline work. Each Electron major has done meaningful OSR-related changes.
3. **Investigate the spike pattern more carefully.** The 60 ms gaps in `dt` on Electron 37 looked vsync-aligned (~4 frame intervals). On Electron 41 they're less frequent. If they remain, `--disable-gpu-vsync` is worth trying — we never did because we got distracted by the `useSharedTexture` rabbit hole.
4. **Don't enable `useSharedTexture` again unless you also implement** the unpremultiply + colorSpace conversion. The blurry-text outcome is unacceptable, and skipping those steps is the entire reason it was blurry.
...
- `src/paint.ts` — paint handler with the `useSharedTexture` branch (currently dormant, falls through to `toBitmap`). All the timing instrumentation lives here. The `tb`/`sw` measurement loop is the workhorse.
- `src/windows.ts` — `webPreferences.offscreen` is the toggle. Currently `true`; flip to `{ useSharedTexture: true }` to re-enable the shared-texture path.
...
- `src/index.ts` — the Chromium switch list (smooth scrolling, GPU rasterization, etc.).
- `awrit-native-rs/src/lib.rs` — `ShmGraphicBuffer` with name rotation, `truncate_tolerant`, IOSurface FFI module (`iosurface_ffi`), `write` and `write_iosurface` methods. `write_iosurface` is currently uncalled from JS but compiled and ready.
...
Each line: `paint:<tag> src=<tex|bmp|fail> fmt=<bgra|rgba|n/a> dt=<ms> tb=<ms> sw=<ms> sz=WxH ni=WxH cs=WxH dl=WxH@x,y`. Where:
- `dt` — gap from previous paint of this content (low = high paint rate)
- `tb` — `toBitmap` or `writeIosurface` cost (the consumption-side bottleneck)
- `sw` — stdout writes for `loadFrame` + `composite` (Kitty pipeline cost)
- `sz` — image size we feed Kitty
- `ni` — `image.getSize()`
- `cs` — `textureInfo.codedSize` (only present with `useSharedTexture`)
- `dl` — destination rect we tell Kitty to composite into
